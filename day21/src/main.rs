...
fn solve(
    ...
    possible_pawns.insert(
        (
            Pawn::new(win_amount, track_length, pos1),
            Pawn::new(win_amount, track_length, pos2),
        ),
        1,
    );
    ...
    while !possible_pawns.is_empty() {
        ...
        for ((pawn1, pawn2), pawns_amount) in possible_pawns.iter() {
            for (dice_result_p1, dice_result_p1_amount) in dice.multi_role(3).iter() {
                ...
                if new_pawn1.update_pos_wins_p(*dice_result_p1) {
                    ...
                    for (dice_result_p2, dice_result_p2_amount) in dice.multi_role(3).iter() {
...
fn solve_part1(file_name: &str) -> u64 {
    ...
    if player1_wins >= player2_wins {
        player2_loses_scores * dice.get_total_roles()
    } else {
        player1_loses_scores * dice.get_total_roles()
    }
}
