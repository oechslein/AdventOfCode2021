...
impl<'p> Alu<'p> {
    ...
    fn load(&self, v: Var) -> i64 {
        self.vars[v as usize]
    }

    fn store(&mut self, v: Var, i: i64) {
...
fn part1<'p>(alu: &Alu<'p>, depth: usize, known: &mut HashSet<Alu<'p>>) -> Option<u64> {
    for i in (1..=9).rev() {
        ...
        } else if let Some(n) = part1(&alu, depth + 1, known) {
...
fn part2<'p>(alu: &Alu<'p>, depth: u8, known: &mut HashSet<Alu<'p>>) -> Option<u64> {
    for i in 1..=9 {
        let mut alu = alu.clone();
        alu.step(i);
        if known.contains(&alu) {
        ...
        } else if let Some(n) = part2(&alu, depth + 1, known) {
            return Some(n * 10 + i as u64);
        }
        known.insert(alu);
    }
...
fn main() {
    ...
    println!("Part 1: {}", rev(part1(&alu, 0, &mut known).unwrap()));
    println!("Part 2: {}", rev(part2(&alu, 0, &mut known).unwrap()));
}
